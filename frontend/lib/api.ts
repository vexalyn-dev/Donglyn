...
export const api = {
  ...
  // Streaming
  getStream: (d: { url: string; server?: string }) =>
    apiRequest<any>("/api/stream", { method: "POST", body: JSON.stringify(d) }),
  proxyOkru: (url: string) => apiRequest<string>(`/api/proxy-okru?url=${encodeURIComponent(url)}`),
  proxyDm: (url: string) => apiRequest<string>(`/api/proxy-dm?url=${encodeURIComponent(url)}`),
  proxyGeneric: (url: string, referer?: string) =>
    apiRequest<string>(`/api/proxy?url=${encodeURIComponent(url)}${referer ? `&referer=${encodeURIComponent(referer)}` : ""}`),
};
